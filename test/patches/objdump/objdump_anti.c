...
//////////////// ANTI-TAINT-STR //////////////////
char newvar_1[strlen(sec->uncompressed_name)];
if (strlen(sec->uncompressed_name) < 30){    
    for (int i=0;i<strlen(sec->uncompressed_name);i++){
        int ch=0;
        int temp = 0;
        int temp2 = 0;
        for (int j=0; j<8;j++){
            temp = sec->uncompressed_name[i];
            temp2 = temp & (1<<j);
            if (temp2 !=0){
                ch |= 1<<j;
            }
    ...
    strncpy(newvar_1, sec->uncompressed_name, strlen(sec->uncompressed_name));
}
//////////////////////////////////////////////////

	if (strcmp (newvar_1, match) == 0)
...
//////////////// ANTI-TAINT-STR //////////////////
char newvar_2[strlen(sought->section_name)];
if (strlen(sought->section_name) < 30){    
    for (int i=0;i<strlen(sought->section_name);i++){
        int ch=0;
        int temp = 0;
        int temp2 = 0;
        for (int j=0; j<8;j++){
            temp = sought->section_name[i];
            temp2 = temp & (1<<j);
            if (temp2 !=0){
                ch |= 1<<j;
            }
    ...
    strncpy(newvar_2, sought->section_name, strlen(sought->section_name));
}
//////////////////////////////////////////////////

  if (strncmp (newvar_2, section->name, len) == 0
      && (section->name[len] == 0
