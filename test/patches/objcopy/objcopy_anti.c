...
//////////////// ANTI-TAINT-STR //////////////////
char newvar_1[strlen(srename->new_name)];
if (strlen(srename->new_name) < 30){    
    for (int i=0;i<strlen(srename->new_name);i++){
        int ch=0;
        int temp = 0;
        int temp2 = 0;
        for (int j=0; j<8;j++){
            temp = srename->new_name[i];
            temp2 = temp & (1<<j);
            if (temp2 !=0){
                ch |= 1<<j;
            }
    ...
    strncpy(newvar_1, srename->new_name, strlen(srename->new_name));
}
//////////////////////////////////////////////////

	if (strcmp (newvar_1, new_name) == 0
	    && srename->flags == flags)
	  return;
...
//////////////// ANTI-TAINT-INT //////////////////
int newvar_2=0; 
if (reverse_bytes > 0 && reverse_bytes < 255)
{
    for (int i=0; i<=255; i++){
        if (i == reverse_bytes){
            newvar_2 = i;
...
//////////////// ANTI-TAINT-STR //////////////////
char newvar_3[strlen(efi)];
if (strlen(efi) < 30){    
    for (int i=0;i<strlen(efi);i++){
        int ch=0;
        int temp = 0;
        int temp2 = 0;
        for (int j=0; j<8;j++){
    ...
    strncpy(newvar_3, efi, strlen(efi));
}
//////////////////////////////////////////////////

      if (strncmp (newvar_3, "bsdrv-", 6) == 0
...
if (strlen(efi) < 30){    
    ...
    strncpy(newvar_4, efi, strlen(efi));
}
//////////////////////////////////////////////////

      if (strncmp (newvar_4, "app-", 4) == 0)
	{
